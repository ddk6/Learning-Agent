...
- 搜索 `notes/` 下的学习资料
- 通过 `/web` 联网搜索公开网页，默认只返回标题、URL 和摘要
- 通过 `fetch_web_page` 读取公开网页正文，并用 `/ask-web` 完成搜索、读取和带来源回答
- 保存和查看学习记忆
...
`/web <query>` 会通过 `web_search` 工具搜索公开网页。当前 MVP 不新增第三方依赖，默认 `WEB_SEARCH_PROVIDER=auto`：优先使用 `TAVILY_API_KEY`，其次使用 `BRAVE_SEARCH_API_KEY`，都没有时走 Tavily keyless 搜索；`duckduckgo` 只作为实验 fallback。

`fetch_web_page` 用于读取单个公开网页正文。`/ask-web <问题>` 是一个确定性编排命令：先搜索，再读取前 2-3 个可访问结果，最后总结答案并附来源。配置了大模型时会用 LLM 生成综合答案；模型未配置或限流失败时，会退回到网页摘录模式。

安全边界：

- 搜索查询会发送到外部搜索服务，不要输入 API Key、隐私数据或本地文件内容。
- `web_search` 只返回搜索结果标题、URL 和摘要。
- `fetch_web_page` 只允许读取 `http/https` 公网 URL，会拦截 localhost、内网 IP、本地域名、非白名单域名、超时和超大响应。
- `/ask-web` 读取网页时会把每个 URL 锁定到搜索结果自身域名，防止重定向到非预期域名。
- 外部网页内容按不可信输入处理，不能作为工具指令执行；总结提示词会明确忽略网页中的 prompt injection。
- `web_search` 和 `fetch_web_page` 的权限等级是 `medium`，LLM Tool Calling 路径默认需要确认；CLI `/web` 与 `/ask-web` 由用户显式触发。
- 后续如果需要更稳定的生产能力，可以固定 provider 为 Tavily、Brave Search API 或模型厂商内置 Web Search。
...
- `web_search` 只访问公网搜索入口，返回搜索结果摘要；外部结果视为不可信输入。
- `fetch_web_page` 只读取公开网页正文，限制协议、域名、超时、响应大小和返回字符数。
